...
void Deposit_102_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Deposit_102_run(uint ctx_index,Circom_CalcWit* ctx);
...
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 24
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[0]); // line circom 24
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[0]); // line circom 35
while(Fr_isTrue(&expaux[0])){
{
...
// load src
Fr_sub(&expaux[1],&circuitConstants[148],&lvar[0]); // line circom 36
// end load src
...
Deposit_102_create(1,0,ctx,"main",0);
Deposit_102_run(0,ctx);
}
